 ...
 * @author lwh
 */
public class CircleProgressBar extends View implements IConstant{
	...
	private void init(AttributeSet attrs, int defStyle) {
		...
		mStepPaint.setColor(Color.rgb(100,113,205));

		mTargetPaint = new Paint();
		mTargetPaint.setAntiAlias(true);
		mTargetPaint.setColor(Color.GRAY);
	...
	protected void onDraw(Canvas canvas) {
		...
		canvas.drawText(mCurrStepNum + "",
				...
				mStepPaint);
		canvas.drawText("目标 "+mMaxStepNum, mWheelRect.centerX() - (mTargetPaint.measureText("目标 "+mMaxStepNum) / 2), mTargetY,
				mTargetPaint);
	}
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		mCircleStrokeWidth = getTextScale(30, min);// 圆弧的宽度
...
//			}
	...
	public void setColor(int red, int green, int blue) {
		mFinishWheelPaint.setColor(Color.rgb(red, green, blue));
	}
